...
class QuizManager  {
    ...
    private var _totalPerguntasCertas = 0
    ...
    var totalPerguntasCertas: Int {
        return _totalPerguntasCertas
    }
    
    func atualizarQuiz(){
        let radonIndex = Int(arc4random_uniform(UInt32(quizes.count)))
        let quizData = quizes[radonIndex]
        quiz = Quiz(pergunta: quizData.question, opcoes: quizData.options, respostaCerta: quizData.correctAnswer)
        
    }
    
    func validarResposta(index: Int) {
        _totalRespostas  += 1
        if quiz.validadorOpcoes(index){
            _totalPerguntasCertas += 1
        }
